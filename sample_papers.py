...
sample_paper_reviews = """
### Reviewer #1
"paper_summary": "This paper introduces FOLIO, a new human-annotated dataset for first-order logic. This dataset is more complicated compared to existing datasets due to larger reasoning depths, a larger number of distinct logic patterns, and a large vocabulary. The authors conduct detailed experiments using LLMs with various settings on this dataset, and shows that there is still a gap between the best-performed LLMs (i.e., 78.1% with Logic-LM) and expert annotations (95.98%), though the best-performed LLMs already outperform non-expert annotations (i.e., 61.82%). In addition, a detailed error analysis indicate some interesting behaviors of LLMs in this new dataset.", 
"summary_of_strengths": "1) a new natural language reasoning dataset annotated by experts for first-order logic problems\n2) some strategies are employed to control the quality during the corpus construction\n3) very solid experiments for benchmarking the performance of LLMs under different settings in this dataset\n4) a detailed error analysis unveil some interesting behaviors of LLMs", 
"summary_of_weaknesses": "I think there are no major weakness but I'd suggest some potential ways to further improve the current version:\n1) The dataset is still a little small compared to most existing datasets, so obtaining more examples would be beneficial.\n2) It would be great if the authors can also evaluate some more advanced open-source LLMs, such as Llama2/Llama3/Mistral etc.\n3) The results of the best open-source LLMs with various prompting strategies may further indicate the difficulty of this dataset", 
"comments_suggestions_and_typos": "I think Table 6 is very interesting since it shows that LLMs may use quite a different way to deal with first-order logic reasoning in natural language compared to the traditional FOL formulas with inference engine. There may be some interesting directions for the future work (I don't think the authors need to do this in this paper):\n1) NL prompting and NL-FOL with inference engine are two common ways to use LLMs for logical reasoning. I think it would be interesting to figure out what types of problems each method is better at, and explore how to combine their advantages together to build a better hybrid system based on the two approaches\n2) A deeper question is to figure out the latent mechanism of NL prompting for FOL since it's quite different from the traditional FOL formula + inference engine."}, 
"scores": {"soundness": 4, "overall_assessment"}}

### Reviewer #2
"paper_summary": "This paper proposes a new benchmark FOLIO, that targets logical reasoning with first-order logic. An extension of NL-FOL leads to a new translation task. For benchmark creation, there is a manual annotation process where experts are involved. The authors tried to relieve the manual burden by introducing a hybrid approach that uses \"syllogisms\". Finally, the FOL and NL are filtered and verified to ensure quality. Experiments are conducted on fine-tuning LMs and prompting LLMs.", 
"summary_of_strengths": "1. This paper introduces a valuable source of logical reasoning benchmark, consisting of expert-written samples annotated with FOL. The FOL annotations could also be of use for research in symbolic reasoning with LLMs.\n2. The paper provides a thorough evaluation of both fine-tuning LMs and prompting LLMs, offering some interesting insights.\n3. The paper is sound, solid, and well-organized.", 
"summary_of_weaknesses": "1. I am mostly concerned with the scale of this dataset since it relied on much human effort. The authors have already tried to scale up the dataset by introducing HybLogic. However, it seems that quality control still needs a lot of human effort, e.g., manually filtering biased and unfaithful samples.\n2. One important contribution is the expert-written dataset. However, I do not see from the paper the criteria for being an expert. Specifically, how do they differ from non-expert people?\n3. Some parts of the paper presentation need improvement. Please see the detailed comments below.", 
"comments_suggestions_and_typos": "1. The features in Table 1 are not precisely summarized. For example, GMAT, LSAT, and NCSE are all real-world resources. Claiming \"real-world\" could be misleading and overlap with \"real-world examples\". I suggest the authors simply put Wikipedia here.\n2. In line 277, how to ensure the sentences are free from grammar errors, and how to make sure the sentences are natural?\n3. I think section 6.1 Human Performance should be included in Table 4 for readers to understand the human ceiling of the task. Again, please specify the composition of expert annotators.\n4. Please consider adding a figure/table for \"human evaluation on model outputs\".\n5. I don't quite agree with the interpretation in lines 555-563. Why does \"scratching by humans\" lead to \"similar logical patterns in the training data\"? I think the model may not be able to see originally curated samples in the training data."}, 
"scores": {"soundness": 4, "overall_assessment": 3}}

### Reviewer #3
"paper_summary": "To assess the logical reasoning abilities of LMs and LLMs in different settings (i.e., fine-tuning and prompting), this paper proposed expert-annotated, logically complex with various depths of FOL reasoning, and diverse FOLIO dataset. The main characteristic of FOLIO is its ability to be represented in terms of formal FOL inference rules. Authors have benchmarked the performance of various fine-runed LMs and prompting LLMs on the FOLIO. Experimental results indicate that FOLIO is still a challenging dataset even for bigger LLMs such as GPT-4.", 
"summary_of_strengths": "- There is a sore need to evaluate LLMs’ logical reasoning ability, specifically, their ability to understand formal logic. This paper achieves that to some extent with first-order logic.\n- FOLIO is an expert-annotated, logically complex, and diverse dataset with various (~0-7) depths which could be very useful for evaluating upcoming LLMs.\n- Each instance in FOLIO can be presented in formal inference rules of FOL that can help conduct more analysis.\n- The findings are interesting and the paper is well-written", 
"summary_of_weaknesses": "- Major analysis/insights from the results in the paper are based on the objective evaluation, however, evaluation w.r.t answer accuracy is not enough to understand the true nature of models’ reasoning abilities. I acknowledge that authors indeed present a small manual analysis of GPT-4’s reasoning chains, however, insights from that study are neither in detail nor surprising. A more thorough analysis of reasoning chains is needed to get more insights from the results such as identifying the types of mistakes models make when following formal rules, and where they falter the most (just to give an idea related to analysis).\n- An example mentioned in Table 2 indicates that the first-order reasoning problems in FOLIO mainly focused on unary predicates. First-order logic is powerful in terms of understanding n-ary relations with multi-variable setting. Why authors did not explore that avenue? More detailed inference rules can be utilized as shown in LogicBench - https://arxiv.org/abs/2404.15522 \n- It would be more insightful to see the results on more open-sourced LLMs such as Mistral since that would strengthen the proposed findings and showcase that they hold across different models.", 
"comments_suggestions_and_typos": "- It would be great if the authors could add a small section at the end indicating the usefulness of the findings and how they can help in improving the models’ reasoning abilities.\n\nSince evaluating and improving the reasoning abilities of LLMs is important, I am willing to increase my scores if the comments are addressed properly. Thanks."}, 
"scores": {"soundness": 3.5, "overall_assessment": 4}}
"""